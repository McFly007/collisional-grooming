...
void getDeltaV(Vec3 v1, Vec3 v2, float &deltaV)
{
	Vec3 dVec = Vec3(v1.x - v2.x, v1.y - v2.y, v1.z - v2.z);
	deltaV = dVec.norm();
}
...
void readDatafile(string line, std::__cxx11::string &file_to_load, float &diameter, float &pericenter)
{
	...
	if (getme.length() > 0)
	{
		pericenter = stof(getme);
		getme = "";
	} // this is optional so we are outputting 999.999 instead
}
...
float getRadialDifference(std::vector<double> &r_prof1, std::vector<double> &r_prof2)
{
	...
	for (int k = 3; k < (int)r_prof1.size(); ++k)
	{
		...
		if (r_prof1[k] > 0.0 && r_prof2[k] > 0.0)
		{
			...
			if (max_diff < radial_difference)
			{
				max_diff = radial_difference;
			}
		}
	}
...
void record_radar_profile(float a, float e, float inc, float vg, float prob, int diameter_index, std::string radar_name)
{
	...
	if (abs(ecc_lon - HE_lon_center) < HE_lon_width && abs(ecc_lat - HE_lat_center) < HE_lat_width)
	{
		radiant_source_name = "HELION";
	}
	if (abs(ecc_lon - NA_lon_center) < NA_lon_width && abs(ecc_lat - NA_lat_center) < NA_lat_width)
	{
		radiant_source_name = "NORTH APEX";
	}
	if (abs(ecc_lon - NT_lon_center) < NT_lon_width && abs(ecc_lat - NT_lat_center) < NT_lat_width)
	{
		radiant_source_name = "NORTH TOROIDAL";
	}
	...
	if (radar_name == "CMOR")
	{
		...
		if (meteors_N_record > SFD_array[diameter_index][1])
		{
			meteors_N_record = SFD_array[diameter_index][1];
		} // We don't want more meteors recorded than the maximum in the bin
		if (meteors_N_record < 0.0)
		{
			meteors_N_record = 0.0;
		} // We don't want to record negative numbers
		...
		if (radiant_source_name == "HELION")
		{
			...
			if (index_a < (int)My_Measurements.CMOR_AH_a_y.size())
				My_Measurements.CMOR_AH_a_y[index_a] += meteors_recorded;
			if (index_ecc < (int)My_Measurements.CMOR_AH_ecc_y.size())
				My_Measurements.CMOR_AH_ecc_y[index_ecc] += meteors_recorded;
			if (index_inc < (int)My_Measurements.CMOR_AH_inc_y.size())
				My_Measurements.CMOR_AH_inc_y[index_inc] += meteors_recorded;
			if (index_vg < (int)My_Measurements.CMOR_AH_vg_y.size())
				My_Measurements.CMOR_AH_vg_y[index_vg] += meteors_recorded;
		}
	}
	...
	if (radar_name == "AMOR")
	{
		...
		if (radiant_source_name == "HELION")
		{
			...
			if (index_a < (int)My_Measurements.AMOR_AH_a_y.size())
				My_Measurements.AMOR_AH_a_y[index_a] += meteors_recorded;
			if (index_ecc < (int)My_Measurements.AMOR_AH_ecc_y.size())
				My_Measurements.AMOR_AH_ecc_y[index_ecc] += meteors_recorded;
			if (index_inc < (int)My_Measurements.AMOR_AH_inc_y.size())
				My_Measurements.AMOR_AH_inc_y[index_inc] += meteors_recorded;
			if (index_vg < (int)My_Measurements.AMOR_AH_vg_y.size())
				My_Measurements.AMOR_AH_vg_y[index_vg] += meteors_recorded;
		}
	}

	if (radar_ion_limit > 1e9)
	{
		::cout << "Error: Incorrect Radar Selected!" << std::endl;
	}
}
...
void pre_init()
{
	...
	for (int k2 = 0; k2 < (int)Integration_Setup_array.size(); k2++)
	{
		...
		{
			if (float(unique_diameters[k3]) == float(Integration_Setup_array[k2][0]))
			{
				tmp_array[k3] += Integration_Setup_array[k2][1];
			};
		}
	}
...
void init()
{
		...
		while (getline(datafile, line, '\n'))
		{
			...
			if (myfile.is_open())
			{
				...
				while (!myfile.eof())
				{
					...
					if (myfile.eof())
						break;
					if (current_integration_time > time_int)
					{
						particle_weight.clear();
						current_integration_time = time_int;
					}
					...
					if (current_integration_time == time_int) // Here we start reading a new file so we start a new vector etc.
					{
						...
						while ((int)particle_weight.size() < parID)
						{
							::printf("Particle %lu mysteriously missing at the very beginning of the datafile => next particle is %d \n", particle_weight.size(), parID);
							particle_weight.push_back(weight);
						}
					}
					// ----
				...
				myfile.close();
				::printf("Inserted %d points to octree\n", total_number_of_particles_integration);
				::fflush(stdout);
				file_cntr++;
			}
			else
	...
	::fprintf(pFile, "\n");
	checkEclipticProfile(pFile1, ecl_profile, ecl_profile_mass, 0, init_reweight);
	::fprintf(pFile1, "\n");
...
void iteration(const double radial_difference)
{
	...
	{
		setting_weight = 0.3 - tanh(log(radial_difference) * 0.1) * 0.3;
		if (setting_weight > 0.3)
			setting_weight = 0.3;
	}
	// setting_weight = 0.1;
				...
				while (true)
				{
					...
					if (results->getRecNum() > 0)
					{
						...
						for (int k = 0; k < rec_iter; ++k)
						{
							...
							if (Qbind > impactor_energy)
							{
								continue;
							} // Binding Energy is larger than the kinetic energy of the impactor => no effect
	...
	if (file.is_open())
	{
		...
		while (std::getline(file, str))
		{
			...
			if (input_cntr == 2)
				Dmid = stof(str);
			if (input_cntr == 3)
				As_modifier = stof(str);
			if (input_cntr == 4)
				Bs = stof(str);
			if (input_cntr == 5)
				intended_cloud_area = stof(str);
			if (input_cntr == 6)
				pericenter_index = stof(str);
			if (input_cntr == 7)
				work_directory = trim(str);
			if (input_cntr == 8)
				output_switch = stoi(str);
			if (input_cntr == 9)
